...
/**
 * LOOKUP step ΓÇö calls lookup_rule for each field and builds a rules map.
 * Map key = field array index (string) ΓÇö ExtractedField has no id field.
...
export async function lookup(
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    // Skip null/empty text fields ΓÇö OCR sometimes emits blank lines
    ...
    // No match ΓåÆ no entry in map ΓåÆ compare() will skip this field silently
  }
