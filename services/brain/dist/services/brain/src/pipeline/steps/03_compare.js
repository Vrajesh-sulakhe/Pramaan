...
function normalizeToBaseUnit(value, unit) {
    ...
    if (u === "per tablet" || u === "per scan" || u === "per test" || u === "per day" || u === "per procedure")
        return value;
    if (u === "per strip")
        return value / 10;
    if (u === "per ml")
 ...
 */
function findRule(fieldText, rules) {
 ...
 */
export function compare(fields, rules) {
    const results = [];
    for (const field of fields) {
        ...
        const normalizedRuleValue = normalizeToBaseUnit(rule.official_value, rule.official_unit);
