 ...
 */
export async function act(cards, invoice_id) {
    ...
    const confidences = gapCards.map((c) => c.source_anchor.ocr_confidence ?? 1.0);
    ...
    if (confFloor >= THRESHOLD) {
        ...
        const hold = billingGateway.placeHold(invoice_id, totalDisputed, evidencePackId, confFloor);
        // Step 7: audit
        auditLog.append({
            t: "hold_placed",
            run_id,
